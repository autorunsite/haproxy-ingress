...
func (c *converter) addEndpoints(svc *api.Service, servicePort string, backend *hatypes.Backend) error {
	if servicePort == "" {
		return fmt.Errorf("port cannot be empty")
	}
	epport := findServicePort(svc, servicePort)
	if epport.String() == "" {
		return fmt.Errorf("port not found: '%s'", servicePort)
	}
	...
	// TODO epport.IntValue() doesn't work if svc.targetPort is a pod's named port
	for _, subset := range endpoints.Subsets {
		for _, port := range subset.Ports {
			ssport := int(port.Port)
			if ssport == epport.IntValue() && port.Protocol == api.ProtocolTCP {
	...
	if c.globalConfig.DrainSupport {
		...
		for _, pod := range pods {
			ep := backend.NewEndpoint(pod.Status.PodIP, epport.IntValue(), pod.Namespace+"/"+pod.Name)
			ep.Weight = 0
		}
	}
	return nil
